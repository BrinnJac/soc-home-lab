...
One of those alerts does cover the `whoami.exe` process creation directly. But
look at what the rule that matched actually says: it describes suspicious shell
execution, not user or account enumeration. Nothing in the alert identifies this
as discovery activity, and nothing distinguishes it from any other process
spawned by a shell in an unusual way.

So the SIEM flagged the *delivery mechanism* and not the *technique*. What
triggered every one of these rules was the process ancestry created by the
Atomic Red Team harness — cmd.exe under cmd.exe under PowerShell. An attacker
running the same enumeration from a process that already looks ordinary would
likely produce nothing at all, because none of these rules are looking at what
the command does.

Worth stating the limit of this finding precisely: I know what these four alerts
say, not that no other rule anywhere in the ruleset could fire on this behavior.
Within this test, no alert named the activity for what it was.
